...
**Architecture:** Keep `kdrive-native-arch` as the pacman identity and add a strict archive-validation boundary to `install.sh`. Add shell contract/red-team fixtures that operate entirely under temporary directories, plus an Arch-scope check over the repository/archive surface.
...
- Keep `pkgname=kdrive-native-arch` for pacman upgrade and rollback continuity.
...
- [ ] Write failing tests for traversal and symlink release archives, using a
  temporary `KDRIVE_RELEASE_BASE_URL=file://...`, checksum manifests, and a
  marker outside the extraction directory.
- [ ] Run the red-team test and confirm the current installer extracts or
  accepts at least one hostile fixture.
- [ ] Add an Arch-scope assertion that tracked files and the generated source
  archive contain no Swift/macOS/Windows/Android source paths.
- [ ] Run the tests again and record the expected failures before changing the
  installer.
...
- [ ] Add a validator that checks every tar member for one exact
  `kdrive-arch/` root, no absolute or `..` path component, no control bytes,
  and no symlink/hardlink entries.
- [ ] Extract with `--no-same-owner --no-same-permissions` and verify the
  resolved `SOURCE_ROOT` remains below `TEMP_ROOT`.
- [ ] Add static contract assertions for the validator and safe tar flags.
- [ ] Run traversal, symlink, checksum, and normal dry-run fixtures; confirm
...
- [ ] Stub only `makepkg`, `pacman`, and `systemctl` inside a temporary PATH;
  redirect `HOME`, XDG paths, and `KDRIVE_INSTALL_ROOT` to the sandbox.
- [ ] Exercise normal install with `--no-start`, failed download, and rollback
  state capture; assert no file appears outside the sandbox and the old unit
  and autostart files are restored.
- [ ] Document `--dry-run`, `--verify`, and rollback as explicit safe checks,
...
- [ ] Run shell syntax, all contract/red-team tests, source archive inspection,
  `makepkg --nobuild --nodeps --cleanbuild`, and `git diff --check`.
- [ ] Run the full native package build only if code affecting the build path
